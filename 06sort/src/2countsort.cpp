...
#include "./2countsort.h"

using ElemT = uint32_t; // Typ prvku, ktere budeme radit
constexpr size_t N_ELEMS = 50'000'000; // Pocet prvku k serazeni
...
int main() {
    ...
    eval("countsort_seq", unsorted, [](auto& vec) { countsort_seq(vec); });
    eval("countsort_par", unsorted, [](auto& vec) { countsort_par(vec); });
}
